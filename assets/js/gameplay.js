...
const fallbackQuestionsDatabase = {
    fsk0: [
        ...
        "Ich habe schon mal... bei einem Film geweint",
        "Ich habe schon mal... eine ganze Packung Chips auf einmal gegessen",
        "Ich habe schon mal... einen Zug oder Bus verpasst",
        "Ich habe schon mal... mich in einem Gebäude verlaufen",
        "Ich habe schon mal... einen fremden Hund einfach gestreichelt",
        "Ich habe schon mal... bei einem Computerspiel gemogelt",
        "Ich habe schon mal... eine Nacht durchgemacht um eine Serie zu schauen",
        "Ich habe schon mal... meinen eigenen Geburtstag vergessen",
        "Ich habe schon mal... den Geburtstag von jemandem vergessen",
        "Ich habe schon mal... etwas Teures aus Versehen kaputt gemacht",
        "Ich habe schon mal... mir einen Tag freigelassen und einfach zuhause geblieben",
        "Ich habe schon mal... meinen eigenen Witz nicht verstanden",
        "Ich habe schon mal... mit einem Tier gesprochen als wäre es ein Mensch",
        "Ich habe schon mal... eine Pflanze durch zu viel Gießen getötet",
        "Ich habe schon mal... im Supermarkt etwas zurückgestellt weil es zu teuer war",
        "Ich habe schon mal... stundenlang nach etwas gesucht das direkt vor mir lag",
        "Ich habe schon mal... bei einem Gesellschaftsspiel gemogelt",
        "Ich habe schon mal... eine Entschuldigung geschrieben und sie nie abgeschickt",
        "Ich habe schon mal... mehr als 3 Stunden am Stück Videos geschaut",
        "Ich habe schon mal... nachts Angst vor einem Geräusch gehabt",
        "Ich habe schon mal... jemanden mit dem falschen Namen angesprochen",
        "Ich habe schon mal... eine ganze Tube Nutella in einer Woche aufgegessen",
        "Ich habe schon mal... in einem fremden Land die falsche Sprache gesprochen",
        "Ich habe schon mal... einen Strafzettel bekommen",
        "Ich habe schon mal... auf einem Familienfoto ein lustiges Gesicht gezogen",
        "Ich habe schon mal... ein Buch nur wegen des Covers gekauft",
        "Ich habe schon mal... eine Woche lang kein Gemüse gegessen",
        "Ich habe schon mal... heimlich Süßigkeiten aus der Packung einer anderen Person gegessen",
        "Ich habe schon mal... einen Streit angefangen weil ich hungrig war",
        "Ich habe schon mal... im Kino eingeschlafen",
        "Ich habe schon mal... etwas Wichtiges vergessen direkt nachdem ich eingekauft bin",
        "Ich habe schon mal... ein Selfie gemacht und es sofort wieder gelöscht",
        "Ich habe schon mal... stundenlang auf Social Media gescrollt ohne wirklich etwas zu tun",
        "Ich habe schon mal... eine Pizza alleine aufgegessen",
        "Ich habe schon mal... ein Lied falsch mitgesungen ohne es zu merken",
        "Ich habe schon mal... jemanden nachgeahmt ohne zu merken dass er daneben stand",
        "Ich habe schon mal... aus Versehen auf die falsche Person gezeigt",
        "Ich habe schon mal... eine Sendung dreimal neu gestartet weil ich eingeschlafen bin",
        "Ich habe schon mal... vergessen warum ich in ein Zimmer gegangen bin",
        "Ich habe schon mal... einen Witz nicht verstanden und trotzdem mitgelacht",
        "Ich habe schon mal... beim Lachen aus dem Mund getrunken",
        "Ich habe schon mal... einen Regentag als Ausrede benutzt um zuhause zu bleiben",
        "Ich habe schon mal... vergessen mein Handy aufzuladen und hatte den ganzen Tag keinen Akku",
        "Ich habe schon mal... einen Umweg genommen weil mir jemand Bekanntes entgegenkam",
        "Ich habe schon mal... einen Sportplan angefangen und nach 5 Minuten aufgehört"
    ],
    fsk16: [
...
async function startNewQuestion() {
    ...
    GameplayModule.currentGame.currentQuestion = question;
    loadQuestion(question);
...
function buildGuessSelectionUI(actualYesCount) {
    ...
    GameplayModule.currentGame.players.forEach(playerName => {
        const button = document.createElement('button');
        button.className = 'guess-player-btn';
        button.type = 'button';
        button.textContent = playerName;
        button.setAttribute('aria-pressed', 'false');
        button.setAttribute('data-player-name', playerName);
        addTrackedEventListener(button, 'click', () => toggleGuessPlayer(playerName));
...
function toggleGuessPlayer(playerName) {
    if (!GameplayModule.currentGame.currentGuessSelection) {
        GameplayModule.currentGame.currentGuessSelection = [];
    }
...
function evaluateGuessSelection() {
    const actualYesPlayers = GameplayModule.currentGame.players.filter(name => GameplayModule.currentGame.currentAnswers[name] === true);
    const guessedPlayers = GameplayModule.currentGame.currentGuessSelection || [];
...
function setupEventListeners() {
    ...
    const guessSubmitBtn = document.getElementById('guess-submit-btn');
    if (guessSubmitBtn) {
        addTrackedListener(guessSubmitBtn, 'click', evaluateGuessSelection);
    }
